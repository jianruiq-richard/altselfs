...
3. Search for and count KOC, KOL, affiliate, reviewer, and community collaboration posts separately across YouTube, Instagram, TikTok, Facebook, X, LinkedIn, Reddit, and Discord. Treat every named platform as a required search surface, not an optional sample. Use the product name, common misspellings, campaign phrases, hashtags, discount codes, referral codes, tracking parameters, and landing-page URLs to discover the posts. Inventory every qualifying in-window collaboration post found on each platform and record an explicit zero-result finding for any platform where none is found.
