...
1. Primary method: always call `altselfs_youtube_competitor_activity` first with the target product/domain and the exact `since` and `until` timestamps. Pass a known `channelId` or expected `channelName` when available, set `includeOfficial: true`, `includeKoc: true`, and `includeShorts: true`, and pass corrected product names, domains, campaign names, or other aliases established during the investigation through `keywords`; do not hard-code them. Treat `official.videos` as official-channel activity. Treat `koc.videos` as KOC or creator promotion candidates, use `promotionSignals` and `promotionConfidence` to distinguish likely sponsorship, affiliate promotion, or conversion activity from organic coverage, and never present a keyword match as proof of payment. Preserve the tool's coverage limitations.
2. Completion rule: a successful tool response containing zero in-range videos is a valid result and must not by itself trigger fallback. Trigger fallback only when the tool is unavailable or disabled, lacks its RapidAPI key, returns a request/provider/rate-limit error, cannot resolve the official channel when official activity is required, or reports endpoint errors that leave the requested YouTube coverage materially incomplete. If only one section fails, use fallback only for that missing section and retain successful tool results.
3. Fallback method: identify candidate keywords from the current investigation. Use Python, not Node.js, to invoke the system `curl` binary through `subprocess` and capture the HTML returned by `curl -L -A 'Mozilla/5.0' 'https://www.youtube.com/results?search_query=<product-name-keyword>&sp=EgIIAw%253D%253D'`. Parse the page's `ytInitialData` and extract `videoRenderer` entries, including `videoId`, title, channel, relative publication date, and view count. For each candidate that may overlap the requested window, use the same Python `subprocess` and system `curl` approach to request `https://www.youtube.com/watch?v=<videoId>`, then parse `ytInitialPlayerResponse` to verify the exact publication date, full description, channel, and view count. Apply the same exact inclusive `since` and `until` timestamps, deduplicate against successful primary-tool results by `videoId`, separate official and KOC candidates using channel evidence, and label the evidence as fallback web collection.
4. Supplementation rule: direct web search may supplement provider omissions, but never skip the primary tool while the YouTube data source is enabled and healthy.
...
For X, Reddit, Facebook, and LinkedIn, you can directly use Python and Google Search. Parallelize tasks where possible, keep the process concise, minimize verification, and prioritize speed over strict accuracy.

## Short-range tool restriction

If the user requests an investigation covering one month or less, never use `altselfs_similarweb_api1`, `altselfs_semrush13`, or `altselfs_ahrefs_url_research`. These tools provide macro-level information at monthly granularity and are not suitable for shorter time ranges. `altselfs_instagram_competitor_activity`, `altselfs_tiktok_api23`, and `altselfs_youtube_competitor_activity` are explicitly allowed and preferred for Instagram, TikTok, and YouTube in short-range investigations. Strictly follow the method above and delegate the prescribed command to Codex to collect acquisition activity across different platforms.
```
