...
export function planFifoAllocations(lots, amountCredits) {
...
export async function createCreditLot(client, input) {
...
export async function consumeCreditLotsFifo(client, input) {
    const amountCredits = Math.max(0, Math.round(input.amountCredits));
    if (amountCredits === 0) {
        return { allocations: [], allocatedCredits: 0, unallocatedCredits: 0 };
    }
    await client.query('select pg_advisory_xact_lock(hashtext($1), hashtext($2))', [input.accountId, input.debitKey]);
    ...
    ].join(' '), [input.accountId, input.debitKey]);
    if (existing.rows.length > 0) {
        const allocations = existing.rows.map((row) => ({
            lotId: String(row.lotId),
            amountCredits: numberValue(row.amountCredits),
        }));
        const allocatedCredits = allocations.reduce((total, allocation) => (total + allocation.amountCredits), 0);
        return {
            allocations,
            allocatedCredits,
    ...
    ].join(' '), [input.accountId, 'ACTIVE']);
    const planned = planFifoAllocations(selected.rows.map((row) => ({
        id: String(row.id),
    ...
    for (const allocation of planned.allocations) {
        ...
        ].join(' '), [allocation.lotId, allocation.amountCredits]);
        await client.query([
...
export async function getPaymentCreditLot(client, paymentId, investorId, lock = false) {
    const result = await client.query([
