...
export async function createPersonalDataDynamictools(config, input) {
    ...
    const tools = [
        {
            ...
            description: 'List the user-connected personal data accounts available to this turn, such as Gmail, Feishu, Instagram/Facebook Meta accounts. Use before private-channel research when you need to know what the user has authorized.',
            inputSchema: {
                type: 'object',
                properties: {
    ...
    if (metaConnections.length > 0) {
        tools.push({
            ...
            description: 'List Facebook Pages and linked Instagram Professional accounts authorized through Meta/Facebook Login. Use before Instagram/Facebook tasks to understand which Page/IG assets are available.',
            inputSchema: {
                type: 'object',
                properties: {
            ...
            description: 'Read recent media from linked Instagram Professional accounts authorized through Meta/Facebook Login. This does not read personal Instagram DMs or ordinary personal-profile feeds.',
            inputSchema: {
                type: 'object',
                properties: {
            ...
            description: 'Read recent posts from Facebook Pages authorized through Meta/Facebook Login. Use for Page publishing history and public-facing activity, not personal Facebook profile feeds.',
            inputSchema: {
                type: 'object',
                properties: {
                    accountId: { type: 'string', description: 'Optional Altselfs Meta connection id.' },
...
async function metaAccountsList(config, context, args) {
...
async function instagramListMedia(config, context, args) {
    ...
    for (const connection of connections) {
        ...
        for (const selected of instagramAccounts.slice(0, 5)) {
            ...
            const profile = await metaGraphFetch(config, token, `${selected.instagram.id}`, {
            ...
            const media = await metaGraphFetch(config, token, `${selected.instagram.id}/media`, {
...
async function facebookPagePosts(config, context, args) {
    const limit = clampNumber(args.limit, 10, 1, 25);
    const connections = await resolveMetaConnections(config, context, args, { allowAll: true, maxAll: 3 });
    const accounts = [];
    for (const connection of connections) {
        ...
        for (const page of pages.slice(0, 5)) {
            ...
            const response = await metaGraphFetch(config, token, `${page.id}/posts`, {
...
async function feishuLarkCli(config, context, args) {
...
async function resolveMetaConnections(config, context, args, options) {
    const connections = await listPersonalConnections(config, { investorId: context.investorId, userId: context.userId, provider: 'meta' });
    if (connections.length === 0)
        throw new Error('No connected Instagram/Facebook Meta account is available for this user.');
    const accountId = readArgString(args.accountId) || readArgString(args.connectionId);
    const accountEmail = (readArgString(args.accountEmail) || readArgString(args.email) || readArgString(args.account)).toLowerCase();
    if (accountId) {
        const matched = connections.find((item) => item.id === accountId);
        if (!matched)
            throw new Error(`Meta account not found for accountId=${accountId}.`);
        return [matched];
    }
    if (accountEmail && accountEmail !== 'all') {
        const matched = connections.find((item) => item.externalAccountId.toLowerCase() === accountEmail ||
        ...
        if (!matched)
            throw new Error(`Meta account not found for accountEmail=${accountEmail}.`);
        return [matched];
    }
    if (options.allowAll)
...
async function loadMetaCredentialPayload(config, connection) {
    const credential = await loadPersonalCredential(config, { investorId: connection.investorId, connectionId: connection.id });
    if (!credential)
        throw new Error(`Credential not found for Meta account ${connection.displayName}.`);
    const payload = decryptCredentialPayload({
        keyProvider: credential.keyProvider,
        encryptedPayload: credential.encryptedPayload,
        encryptedDataKey: credential.encryptedDataKey,
    ...
    if (payload.provider !== 'meta')
        throw new Error(`Credential provider mismatch for Meta account ${connection.displayName}.`);
    return payload;
}
function assertMetaTokenUsable(payload, connection) {
    const expiresAt = payload.expiresAt ? Date.parse(payload.expiresAt) : 0;
    if (expiresAt && expiresAt <= Date.now() + 60_000) {
        throw new Error(`Meta account ${connection.displayName} needs reconnect: Facebook Login access token is expired.`);
    }
    if (!payload.accessToken)
        throw new Error(`Meta account ${connection.displayName} needs reconnect: no access token is available.`);
}
function selectMetaPages(payload, args) {
    ...
    if (pageName)
        pages = pages.filter((page) => (page.name || '').toLowerCase().includes(pageName));
    if (pages.length === 0) {
...
function selectInstagramAssets(payload, args) {
    ...
    for (const page of payload.pages || []) {
        ...
        if (!instagram)
            continue;
        if (igUserId && instagram.id !== igUserId)
            continue;
        if (username && (instagram.username || '').toLowerCase() !== username)
            continue;
...
async function metaGraphFetch(config, accessToken, path, params) {
    ...
    if (!res.ok)
        throw new Error(`Meta Graph API failed (${normalizedPath}): ${JSON.stringify(data).slice(0, 1200)}`);
    return data;
}
...
async function runFeishuCliRawForConnection(config, context, connection, args, options = {}) {
    const { profileName, payload } = await loadFeishuCliCredential(config, context, connection);
...
function sanitizeMetaProfile(profile, payload) {
    return {
...
function sanitizeMetaInstagram(account) {
    return {
        id: account.id,
...
function readStringArrayArg(value) {
    if (!Array.isArray(value))
        throw new Error('args must be an array of lark-cli argument strings.');
    if (value.length === 0)
        throw new Error('args cannot be empty.');
    if (value.length > LARK_CLI_ARGS_MAX_ITEMS)
        throw new Error(`args has too many items; max ${LARK_CLI_ARGS_MAX_ITEMS}.`);
    const result = [];
    let totalLength = 0;
    for (const item of value) {
        ...
        if (!trimmed)
            throw new Error('args cannot contain empty strings.');
        if (trimmed.length > LARK_CLI_ARG_MAX_CHARS)
            throw new Error(`lark-cli argument is too long; max ${LARK_CLI_ARG_MAX_CHARS} chars.`);
        totalLength += trimmed.length;
        if (totalLength > LARK_CLI_ARGS_TOTAL_MAX_CHARS)
            throw new Error(`lark-cli arguments are too long; max ${LARK_CLI_ARGS_TOTAL_MAX_CHARS} total chars.`);
        result.push(trimmed);
    }
    return result;
}
function validateLarkCliArgs(args) {
    if (args[0] === 'lark-cli')
        throw new Error('Pass only arguments after lark-cli; do not include the lark-cli binary name.');
    if (args[0].startsWith('-'))
        throw new Error('Start with a lark-cli command such as skills, schema, drive, docs, calendar, im, api, auth, config, or profile.');
}
