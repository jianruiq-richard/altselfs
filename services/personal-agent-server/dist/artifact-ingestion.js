...
import { getAgentContextArtifactsByIds, persistAgentArtifacts, } from './agent-context-store.js';
import { buildGeneratedObjectKey, downloadObjectToBuffer, isArtifactObjectStorageConfigured, uploadBufferToObject, } from './artifact-storage.js';
import { id, isRecord, nowIso, truncate } from './util.js';
export async function ingestWorkspaceAttachments(config, request, paths, runId) {
    const attachments = getWorkspaceAttachments(request);
    const artifactRefs = getWorkspaceArtifactRefs(request);
    const warnings = [];
    if (attachments.length === 0 && artifactRefs.length === 0)
    ...
    for (const [index, attachment] of attachments.entries()) {
        try {
            ...
            indexLines.push(`- ${attachment.name || basename}`, `  - path: ${metadata.relativePath}`, parsedTextPath ? `  - parsed_text: ${metadata.parsedTextRelativePath}` : '  - parsed_text: unavailable', metadata.annotationHash ? `  - annotation_hash: ${metadata.annotationHash}` : '', metadata.pdfTextStats ? `  - pdf_text_stats: ${formatPdfTextStats(metadata.pdfTextStats)}` : '', `  - mime_type: ${artifact.mimeType || 'unknown'}`, `  - size_bytes: ${artifact.sizeBytes || 0}`, '');
        }
        catch (error) {
...
export async function collectGeneratedWorkspaceArtifacts(config, request, paths, runId, startedAtMs) {
    const warnings = [];
...
async function ingestObjectStorageArtifact(input) {
    ...
    if (!objectKey)
        throw new Error('missing OSS object key');
    ...
    input.indexLines.push(`- ${artifact.name || basename}`, `  - path: ${metadata.relativePath}`, parsedTextPath ? `  - parsed_text: ${metadata.parsedTextRelativePath}` : '  - parsed_text: unavailable', metadata.annotationHash ? `  - annotation_hash: ${metadata.annotationHash}` : '', metadata.pdfTextStats ? `  - pdf_text_stats: ${formatPdfTextStats(metadata.pdfTextStats)}` : '', `  - mime_type: ${artifact.mimeType || 'unknown'}`, `  - size_bytes: ${artifact.sizeBytes || 0}`, '');
    return artifact;
}
async function listRecentOutputFiles(root, startedAtMs) {
    const results = [];
    async function walk(dir) {
        ...
        for (const entry of entries) {
            ...
            if (stat.size <= 0 || stat.mtimeMs < startedAtMs - 1000)
                continue;
            results.push({
                path: fullPath,
...
function getWorkspaceAttachments(request) {
...
function getWorkspaceArtifactRefs(request) {
...
function buildArtifactFileName(index, name, mimeType) {
...
async function parseAttachment(config, attachment, decoded, filePath) {
...
function isLocalStructuredFile(mimeType, name) {
    const lowerName = name.toLowerCase();
...
async function parseWithLocalPython(filePath, mimeType, name) {
...
function runPythonArtifactParser(input) {
    return new Promise((resolve, reject) => {
        ...
        child.on('close', (code) => {
            ...
            try {
                ...
                if (!isRecord(parsed))
                    throw new Error('parser output is not an object');
...
function formatPdfTextStats(stats) {
    return [
...
`;
async function callOpenRouterImageParser(config, image) {
    const apiKey = process.env[config.openRouterApiKeyEnv]?.trim();
    if (!apiKey)
        return '';
    ...
    if (!text)
        throw new Error('OpenRouter image parser returned empty text');
    return text.slice(0, readParserMaxChars());
}
export async function callOpenRouterFileParser(config, file) {
    ...
    if (!text)
        throw new Error('OpenRouter file parser returned empty text');
    return {
...
export function extractFileAnnotation(payload) {
    if (!isRecord(payload))
        return { text: '', hash: '' };
...
function extractGenerationId(payload, response) {
...
function readGeneratedMaxFileBytes(config) {
