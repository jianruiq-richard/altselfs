...
export async function ingestWorkspaceAttachments(config, request, paths, runId) {
    ...
    for (const [index, attachment] of attachments.entries()) {
        try {
            ...
            indexLines.push(`- ${attachment.name || basename}`, `  - path: ${metadata.relativePath}`, parsedTextPath ? `  - parsed_text: ${metadata.parsedTextRelativePath}` : '  - parsed_text: unavailable', `  - mime_type: ${artifact.mimeType || 'unknown'}`, `  - size_bytes: ${artifact.sizeBytes || 0}`, '');
        }
        catch (error) {
...
async function ingestObjectStorageArtifact(input) {
    ...
    input.indexLines.push(`- ${artifact.name || basename}`, `  - path: ${metadata.relativePath}`, parsedTextPath ? `  - parsed_text: ${metadata.parsedTextRelativePath}` : '  - parsed_text: unavailable', `  - mime_type: ${artifact.mimeType || 'unknown'}`, `  - size_bytes: ${artifact.sizeBytes || 0}`, '');
    return artifact;
}
...
async function callOpenRouterFileParser(config, file) {
    const response = await fetch(`${config.openRouterBaseUrl.replace(/\/$/, '')}/chat/completions`, {
        method: 'POST',
        headers: {
