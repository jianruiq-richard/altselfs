...
export async function settleRunCredits(config, input) {
    ...
    if (!usage)
        return { settled: false, reason: 'usage_unavailable' };
    ...
    return runSerializableBillingTransaction(config, async (client) => {
        ...
        await client.query([
            ...
            'where id = $1',
        ].join(' '), [row.accountId, nextBalance, nextReserved, billedCredits]);
        await client.query([
            'update credit_reservations',
            'set status = $2, "capturedCredits" = $3, "shortfallCredits" = $4,',
            '"settledAt" = now(), "updatedAt" = now()',
            'where id = $1',
        ].join(' '), [row.id, settledStatus, billedCredits, shortfallCredits]);
        await client.query([
...
export async function releaseRunCredits(config, input) {
    const pool = getBillingPool(config);
    if (!pool)
        return { released: false, reason: 'database_unavailable' };
    return runSerializableBillingTransaction(config, async (client) => {
        const selected = await client.query([
            'select r.id, r."accountId", r."investorId", r."threadId", r.mode,',
            'r."reservedCredits", a."balanceCredits", a."reservedCredits" as "accountReservedCredits"',
        ...
        const nextReserved = Math.max(0, row.accountReservedCredits - row.reservedCredits);
        await client.query('update credit_accounts set "reservedCredits" = $2, "updatedAt" = now() where id = $1', [row.accountId, nextReserved]);
        await client.query([
            'update credit_reservations',
            'set status = $2, "settledAt" = now(), "updatedAt" = now()',
            'where id = $1',
        ].join(' '), [row.id, 'RELEASED']);
        await client.query([
            'insert into credit_ledger_entries',
            '("id", type, "amountCredits", "reservedDeltaCredits", "balanceAfterCredits",',
            '"reservedAfterCredits", description, "idempotencyKey", "runId", "threadId", metadata,',
...
function extractAgentRunUsage(raw) {
    if (!isRecord(raw) || !isRecord(raw.usage))
        return null;
    const usage = raw.usage;
    if (!isRecord(usage.hermes) || !isRecord(usage.codex))
        return null;
...
function normalizeReservationRow(value) {
    if (!value)
        return null;
    return {
        id: String(value.id || ''),
