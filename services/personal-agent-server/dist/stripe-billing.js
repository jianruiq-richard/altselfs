...
async function createStripeCancellationPortal(config, input) {
    const investorId = requiredString(input.investorId, 'investorId');
    const stripe = requiredStripe(config);
...
async function ensureStripeBillingManagementPortalConfiguration(config, stripe) {
    ...
    const configurationId = configurations.data.find((configuration) => (configuration.metadata?.altselfsPurpose === purpose
        && configuration.metadata?.altselfsVersion === version))?.id || null;
    const features = {
        customer_update: {
            enabled: true,
    ...
    const configuration = await stripe.billingPortal.configurations.create(params, { idempotencyKey: `altselfs:portal-configuration:billing-management:${version}` });
    return configuration.id;
}
export function buildStripeUpgradePortalFeatures(target) {
    return {
        invoice_history: { enabled: true },
        payment_method_update: { enabled: true },
        subscription_update: {
...
async function ensureStripeUpgradePortalConfiguration(config, stripe, planKey, priceId) {
    ...
    const configurationId = configurations.data.find((configuration) => (configuration.metadata?.altselfsPurpose === purpose
        ...
        && configuration.metadata?.altselfsPriceId === priceId))?.id || null;
    const price = await stripe.prices.retrieve(priceId);
    const productId = stripeId(price.product);
    if (!productId) {
        throw new StripeBillingError(409, 'STRIPE_PRICE_PRODUCT_MISSING', `Stripe price ${priceId} is not attached to a product.`);
    }
...
export async function changeStripePlan(config, input) {
    ...
    const targetRank = PLAN_CATALOG[targetPlanKey].rank;
    ...
    if (targetRank < currentRank) {
        throw new StripeBillingError(409, 'PLAN_DOWNGRADE_NOT_SUPPORTED', 'Plan downgrades are not available. Cancel the current subscription, then choose a new plan after the billing period ends.', { currentPlanKey, targetPlanKey });
    }
    ...
    const configurationId = await ensureStripeUpgradePortalConfiguration(config, stripe, planKey, priceId);
...
async function upsertSubscriptionFromStripe(client, investorId, subscription, planKey, period, invoiceId) {
    ...
    const cancellationScheduled = !deleted && (subscription.cancel_at_period_end === true ||
        typeof subscription.cancel_at === 'number');
    await client.query([
        'update credit_subscriptions set "planKey" = $2, status = $3, "monthlyCredits" = $4,',
