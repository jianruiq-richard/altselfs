...
export async function buildAgentRunUsage(input) {
    ...
    return {
        ...
        totalCredits: Math.max(input.config.creditsMinimumRunCharge, pricedHermes.credits + codexCredits),
    };
}
...
function priceHermesUsage(config, usage, model, provider) {
...
function resolveHermesPricing(config, provider, model) {
...
function calculateHermesCostUsd(usage, pricing) {
    const cost = (usage.inputTokens * pricing.inputUsdPerMillion +
        ...
        usage.cacheWrite1hTokens * pricing.cacheWrite1hUsdPerMillion) / 1_000_000;
    return cost * pricing.multiplier;
}
function resolveHermesCostSource(usage, locallyEstimatedCostUsd) {
    if (usage.actualCostUsd > 0)
        return 'provider_actual';
    if (locallyEstimatedCostUsd > 0)
...
function subtractHermesUsage(after, before) {
    return {
        ...
        cacheWriteUnclassifiedTokens: positiveDelta(after.cacheWriteUnclassifiedTokens, before.cacheWriteUnclassifiedTokens),
...
async function readHermesUsageLog(logPath) {
    if (!logPath)
        return { ...EMPTY_HERMES_USAGE };
...
function normalizeRawHermesUsage(rawCompletion, provider) {
    ...
    if (normalizedProvider === 'apiyi') {
        ...
        const cacheWriteTokens = Math.max(numberValue(usage.cache_creation_input_tokens), cacheWrite5mTokens + cacheWrite1hTokens);
...
function normalizeHermesUsageRecord(record) {
    ...
    const cacheWriteTokens = Math.max(numberValue(record.cache_write_tokens), cacheWrite5mTokens + cacheWrite1hTokens);
    return classifyUnspecifiedApiyiCacheWrites({
        ...EMPTY_HERMES_USAGE,
...
function classifyUnspecifiedApiyiCacheWrites(usage, provider) {
...
function addHermesUsage(target, value) {
...
function extractProviderCostUsd(rawCompletion, usage) {
    const candidates = [
        usage.cost,
        usage.total_cost,
