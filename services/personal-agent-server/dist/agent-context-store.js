...
export async function persistAgentTurnRejected(config, input) {
    const pool = await getRequiredContextPool(config);
    await pool.query([
...
export async function requestAgentRunCancellation(config, input) {
    const pool = await getRequiredContextPool(config);
    ...
    ].join(' '), [input.runId, input.threadId || null, input.investorId || null, reason]);
    ...
    if (!row) {
        const existing = await pool.query([
            ...
            'and ($3::text is null or investor_id = $3)',
            'limit 1',
        ].join(' '), [input.runId, input.threadId || null, input.investorId || null]);
        return {
...
export async function listRequestedAgentRunCancellations(config, runIds) {
...
export async function expireStaleAgentTurns(config, input) {
    ...
    await Promise.all(result.rows.map((row) => enqueueAgentBillingEvent(pool, {
        runId: String(row.id || ''),
        action: 'RELEASE',
...
export async function processAgentBillingOutbox(config, input) {
    const pool = await getRequiredContextPool(config);
    ...
    for (const event of claimed) {
        try {
            ...
            ].join(' '), [event.runId, input.workerId]);
            completed += 1;
        }
        catch (error) {
            ...
            await pool.query([
                ...
                'where run_id = $1 and locked_by = $2',
            ].join(' '), [event.runId, input.workerId, terminal ? 'FAILED' : 'PENDING', truncate(message, 4000), retryDelayMs]);
            failed += 1;
        }
    }
...
async function enqueueAgentBillingEvent(pool, input) {
    if (!input.runId)
        return;
    await pool.query([
        ...
        'updated_at = now()',
    ].join(' '), [input.runId, input.action, stringifyJson(input.payload)]);
}
async function backfillAgentBillingOutbox(pool) {
    await pool.query([
        'insert into agent_billing_outbox',
        '(run_id, action, payload, status, available_at)',
        'select r.id,',
...
async function claimAgentBillingEvents(pool, input) {
    const result = await pool.query([
        'with candidates as (',
        'select run_id',
...
async function executeAgentBillingEvent(config, event) {
    if (event.action === 'RELEASE') {
        ...
        if (released.released || released.reason === 'reservation_not_active')
            return;
    ...
    if (settled.settled || settled.reason === 'reservation_not_active')
        return;
...
function billingUsageEnvelope(raw) {
