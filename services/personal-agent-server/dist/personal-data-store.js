...
import { normalizeFeishuCliFeaturePackages } from './feishu-cli.js';
import { id } from './util.js';
let sharedPool = null;
let sharedUrl = '';
let schemaReady = null;
export function isPersonalDataStoreConfigured(config) {
    return Boolean((config.contextDatabaseUrl || config.databaseUrl || '').trim());
}
export async function listPersonalConnections(config, input) {
...
export async function upsertGmailOAuthConnection(config, input) {
    const pool = await getPersonalDataPool(config);
    ...
    const existing = await pool.query([
        ...
        'limit 1',
    ].join(' '), [input.investorId, 'gmail', accountEmail, 'active']);
    ...
    if (!refreshToken && existing.rows[0]?.encrypted_payload && existing.rows[0]?.encrypted_data_key && existing.rows[0]?.key_provider) {
        const current = decryptCredentialPayload({
...
export async function upsertMetaOAuthConnection(config, input) {
    const pool = await getPersonalDataPool(config);
    const capabilityId = await upsertCapability(pool, {
        investorId: input.investorId,
        userId: input.userId,
    ...
    if (!accountId)
        throw new Error('Meta accountId is required.');
    ...
    const existing = await pool.query([
        ...
        'limit 1',
    ].join(' '), [input.investorId, 'meta', accountId]);
    const connectionId = existing.rows[0]?.id ? String(existing.rows[0].id) : id('conn');
    const encrypted = encryptCredentialPayload({
        provider: 'meta',
        accessToken: input.token.accessToken,
...
export async function upsertFeishuOAuthConnection(config, input) {
    const pool = await getPersonalDataPool(config);
    const capabilityId = await upsertCapability(pool, {
        investorId: input.investorId,
        userId: input.userId,
    ...
    const existing = await pool.query([
        ...
        'where c.investor_id = $1 and c.provider = $2 and c.external_account_id = $3',
        'limit 1',
    ].join(' '), [input.investorId, 'feishu', accountId, 'active']);
    const connectionId = existing.rows[0]?.id ? String(existing.rows[0].id) : id('conn');
    let refreshToken = input.token.refreshToken;
    if (!refreshToken && existing.rows[0]?.encrypted_payload && existing.rows[0]?.encrypted_data_key && existing.rows[0]?.key_provider) {
        const current = decryptCredentialPayload({
...
export async function updateFeishuConnectionFeaturePackages(config, input) {
    const pool = await getPersonalDataPool(config);
    const featurePackages = normalizeFeishuCliFeaturePackages(input.featurePackages, []);
    const values = [input.investorId];
    const userId = input.userId?.trim();
    ...
    if (!row)
        return null;
    if (String(row.provider) !== 'feishu')
        throw new Error('Only Feishu connections support feature package updates.');
    ...
    ].join(' '), [ownerInvestorId, input.connectionId, JSON.stringify(metadata)]);
...
export async function disablePersonalConnection(config, input) {
    ...
    ].join(' '), [ownerInvestorId, input.connectionId]);
    return true;
}
export async function loadPersonalCredential(config, input) {
    const pool = await getPersonalDataPool(config);
    const result = await pool.query([
        ...
        'limit 1',
    ].join(' '), [input.investorId, input.connectionId]);
...
export async function updatePersonalCredentialPayload(config, input) {
...
async function upsertCapability(pool, input) {
    ...
    const result = await pool.query([
        ...
        'returning id',
    ].join(' '), [capabilityId, input.investorId, input.userId, input.capabilityKey, input.capabilityType, input.displayName]);
    return String(result.rows[0].id);
}
async function getPersonalDataPool(config) {
    const connectionString = (config.contextDatabaseUrl || config.databaseUrl || '').trim();
    if (!connectionString)
        throw new Error('AGENT_CONTEXT_DATABASE_URL or DATABASE_URL is required for personal data credentials.');
    if (sharedPool && sharedUrl === connectionString)
        return sharedPool;
    let pg;
    try {
        pg = (await import('pg'));
    ...
    if (!schemaReady)
        schemaReady = createPersonalDataSchema(sharedPool);
    await schemaReady;
    return sharedPool;
}
async function createPersonalDataSchema(pool) {
    await pool.query(`
...
function normalizeMetaPages(value) {
    return value.map((item) => normalizeMetaPage(item)).filter(Boolean);
}
function normalizeMetaPage(value) {
...
function normalizeMetaInstagramAccounts(value, pages) {
    const accounts = value.map((item) => normalizeMetaInstagramAccount(item, undefined)).filter(Boolean);
...
function normalizeMetaInstagramAccount(value, page) {
    if (!isPlainRecord(value))
        return null;
    const idValue = readRecordString(value, 'id');
    if (!idValue)
...
function sanitizeMetaInstagramAccount(account) {
    return {
        id: account.id,
...
function sanitizeRecord(value) {
    return Object.fromEntries(Object.entries(value).filter(([key]) => !/token|secret|authorization/i.test(key)));
}
