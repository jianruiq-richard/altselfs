...
export async function getAdminBillingDetail(config, investorId) {
...
export async function adjustAdminBillingCredits(config, input) {
    ...
    return runSerializableBillingTransaction(config, async (client) => {
        ...
        if (action === 'DEDUCT' && nextAvailableCredits < 0) {
            throw new AdminBillingError(409, 'INSUFFICIENT_AVAILABLE_CREDITS', 'This deduction would make available credits negative.', {
                balanceCredits: account.balanceCredits,
                reservedCredits: account.reservedCredits,
...
export async function updateAdminBillingSubscription(config, input) {
    const investorId = normalizeRequiredString(input.investorId, 'investorId');
    const planKey = normalizeRequiredString(input.planKey, 'planKey').toUpperCase();
    const plan = planFor(planKey);
    if (!ADMIN_PLAN_LIMITS[planKey])
        throw new AdminBillingError(400, 'UNSUPPORTED_PLAN', 'Unsupported plan.');
...
async function ensureUserExists(client, investorId) {
    const result = await client.query('select id from users where id = $1', [investorId]);
    if (!result.rows[0])
        throw new AdminBillingError(404, 'USER_NOT_FOUND', 'User not found.');
}
async function ensureAdminBillingAccount(config, client, investorId) {
...
async function ensureFreeSubscription(client, investorId) {
    ...
    ].join(' '), [id('credit_subscription'), 'FREE', 'ACTIVE', ADMIN_PLAN_LIMITS.FREE.monthlyCredits, investorId]);
}
async function getSubscription(client, investorId) {
    ...
    if (result.rows[0])
        return subscriptionRow(result.rows[0]);
    await ensureFreeSubscription(client, investorId);
...
function subscriptionSnapshot(value) {
    return {
...
function normalizeReason(value) {
    ...
    if (!reason)
        throw new AdminBillingError(400, 'INVALID_REASON', 'Reason is required.');
    if (reason.length > 500)
        throw new AdminBillingError(400, 'INVALID_REASON', 'Reason must be 500 characters or fewer.');
    return reason;
}
...
function normalizeAdminActor(value) {
...
function parseDateOrNull(value) {
    ...
    if (typeof value !== 'string')
        throw new AdminBillingError(400, 'INVALID_DATE', 'Date must be an ISO string.');
    const date = new Date(value);
    if (Number.isNaN(date.getTime()))
        throw new AdminBillingError(400, 'INVALID_DATE', 'Invalid date.');
    return date;
}
...
function cleanJson(value, depth = 0) {
    ...
    if (Array.isArray(value))
        return value.slice(0, 120).map((item) => cleanJson(item, depth + 1));
    if (typeof value === 'object') {
        if ('toJSON' in value && typeof value.toJSON === 'function')
            return cleanJson(value.toJSON(), depth + 1);
