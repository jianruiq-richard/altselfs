...
if (config.processRole === 'api' || config.processRole === 'all') {
    ...
    server.listen(config.port, () => {
        ...
        console.log(`[personal-agent-server] runtimeStateSync=${config.runtimeStateSyncEnabled ? 'enabled' : 'disabled'} mode=${config.runtimeStateMode} cacheTtlMs=${config.runtimeStateCacheTtlMs}`);
    });
}
