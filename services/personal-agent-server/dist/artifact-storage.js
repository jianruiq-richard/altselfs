...
export function isArtifactObjectStorageConfigured(config) {
...
export function requireArtifactObjectStorage(config) {
    const credentials = getObjectStorageCredentials(config);
    if (!config.artifactObjectStorageEnabled) {
...
export function createDirectUploadPolicy(config, input) {
...
export function createSignedObjectUrl(config, input) {
    const credentials = requireArtifactObjectStorage(config);
...
export async function downloadObjectToBuffer(config, objectKey) {
...
export async function uploadBufferToObject(config, input) {
    const credentials = requireArtifactObjectStorage(config);
...
function buildUploadedObjectKey(input) {
...
function createSignedObjectUrlForEndpoint(config, input) {
    const credentials = requireArtifactObjectStorage(config);
...
function objectStorageBucketUrl(config, options) {
...
function getObjectStorageCredentials(config) {
    return {
...
function signHmacSha1Base64(secret, value) {
