...
export function createSandboxExecDynamictool() {
  return {
    ...
    description:
      `Run a short Python or shell command in the current ${PRODUCT_BRAND.name} sandbox workspace when deterministic computation, parsing, scraping, or file transformation is needed. The command runs in an isolated Docker container with limited CPU, memory, process count, timeout, and workspace-only filesystem access. PDF profile: inspect \`artifacts/index.md\` and any existing parsed Markdown first. Local tools should only extract candidate text and facts: use Poppler (\`pdfinfo\`, then \`pdftotext -layout\`) first, then pypdf/pdfplumber for cross-checks. Do not treat numeric density or page-coverage stats as the final completeness decision; use model judgment after reading the extracted正文, checking page markers, repeated header/footer patterns, and the fields the user asked about. If the local text appears incomplete, scanned, header/footer-only, or missing user-requested正文, call \`altselfs_pdf_openrouter_parse\` when available. Its OpenRouter prompt is parse-only: “只解析当前文档并返回短确认，不要总结和思考。” It uses OpenRouter file-parser with Mistral OCR by default, extracts file annotations rather than model replies, and saves full Markdown plus annotation hash. Do not rerun OCR when a parsed Markdown artifact already exists. Local page-scoped RapidOCR is the final fallback only when OpenRouter parsing is unavailable or failed, and must be batched to avoid memory pressure. Prefer registered platform tools for third-party data. Do not use this for repository edits or package builds.`,
    inputSchema: {
      type: 'object',
      properties: {
        command: {
        ...
        useProxy: {
          type: 'boolean',
          description:
            'Set true only when network access is needed and the server has configured a proxy/VPN for sandbox execution.',
        },
      },
      required: ['command'],
...
async function runInDockerSandbox(input: {
  ...
  if (args.stdin) await fs.writeFile(stdinPath, args.stdin, 'utf8');
  ...
  try {
    const create = await dockerRequest<Record<string, unknown>>(
      config,
      'POST',
      `/containers/create?name=${encodeURIComponent(containerName)}`,
      dockerContainerCreateBody({
        args,
        config,
        workspace,
    ...
    if (!containerId) throw new Error(`Docker did not return a container id: ${create.bodyText.slice(0, 500)}`);

    await dockerRequest(config, 'POST', `/containers/${containerId}/start`, undefined, 15_000);
    const waitPromise = dockerRequest<Record<string, unknown>>(
      config,
      'POST',
    ...
    if (containerId) {
      await dockerRequest(config, 'DELETE', `/containers/${containerId}?force=true&v=false`, undefined, 10_000).catch(
        () => null
      );
    }
  }
...
function buildRunnerScript(input: { cwd: string; execRelative: string; hasStdin: boolean }) {
...
function parseSandboxExecArgs(argumentsValue: unknown, config: ServerConfig): SandboxExecArgs | { error: string } {
  const args = isRecord(argumentsValue) ? argumentsValue : {};
  const command = typeof args.command === 'string' ? args.command.trim() : '';
  if (!command) return { error: 'command is required' };
  if (Buffer.byteLength(command, 'utf8') > 64 * 1024) return { error: 'command is too large' };
  const cwd = normalizeRelativeWorkspacePath(typeof args.cwd === 'string' ? args.cwd : '.');
  if (!cwd) return { error: 'cwd must be a relative path inside /workspace' };
  const stdin = typeof args.stdin === 'string' ? args.stdin : '';
  if (Buffer.byteLength(stdin, 'utf8') > 1024 * 1024) return { error: 'stdin is too large' };
...
function normalizeRelativeWorkspacePath(value: string) {
  ...
  if (normalized.startsWith('/') || normalized === '..' || normalized.startsWith('../')) return '';
  return normalized;
}

function resolveWorkspace(config: ServerConfig, context: SandboxExecContext): { workspace: string } | { error: string } {
  ...
  const fallback = context.userId
    ? path.join(
        config.sandboxStorageRoot,
        'users',
        sanitizePathSegment(context.userId),
        'threads',
        sanitizePathSegment(context.threadId || 'default'),
        'workspace'
      )
    : '';
  const workspace = explicit || fallback;
  if (!workspace) return { error: 'sandbox workspace context is missing' };
...
function isAllowedWorkspace(config: ServerConfig, workspace: string) {
...
async function ensureSandboxWorkspaceLayout(workspace: string) {
...
function dockerSocketAvailable(socketPath: string) {
...
async function readOutputFile(filePath: string, maxBytes: number) {
...
async function readExitCode(filePath: string, timedOut: boolean) {
...
async function appendCommandAudit(workspace: string, payload: Record<string, unknown>) {
...
function publicContext(context: SandboxExecContext) {
  return {
    userId: context.userId || null,
    threadId: context.threadId || null,
    runId: context.runId || null,
