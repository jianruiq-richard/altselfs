  ...
  if (feishuConnections.length > 0) {
    tools.push(
      ...
      },
      {
        ...
        description:
          'Search Feishu/Lark IM messages across chats with the user-authorized lark-cli profile. Prefer this for questions about a person, keyword, today\'s messages, mentions, or follow-ups because it does not require a prior chat list.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Search Feishu/Lark contacts by name/email/open id with the user-authorized lark-cli profile. Use before reading a direct conversation by person name; results may include p2p_chat_id/open_id.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'List Feishu/Lark IM chats visible through the user-authorized lark-cli profile. Includes p2p and group chats when Feishu returns them. Prefer search_messages for person/topic questions.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Read messages from one Feishu/Lark chat or direct user conversation with the user-authorized lark-cli profile. Provide chatId/containerId or a direct userId/open_id.',
        inputSchema: {
          type: 'object',
          properties: {
      ...
      },
      {
        ...
        description:
          'Read the authorized user\'s Feishu/Lark calendar agenda for a date window with lark-cli. Use for today\'s meetings, schedule, and time commitments.',
        inputSchema: {
          type: 'object',
          properties: {
...
async function feishuSearchUsers(config: ServerConfig, context: PersonaltoolContext, args: Record<string, unknown>) {
  ...
  if (!query && !queries && !userIds) throw new Error('query, queries, or userIds is required.');
...
async function resolveFeishuConnections(
  ...
  if (accountEmail && accountEmail !== 'all') {
    const matched = allConnections.find((item) =>
      item.externalAccountId.toLowerCase() === accountEmail ||
      item.displayName.toLowerCase() === accountEmail
    );
...
function hasFeishuFeaturePackage(connection: PersonalConnection, featurePackage: FeishuCliFeaturePackage) {
  if (connection.provider !== 'feishu' || connection.connectionType !== 'lark_cli_user') return false;
...
async function loadFeishuCliCredential(config: ServerConfig, context: PersonaltoolContext, connection: PersonalConnection) {
  ...
  if (!profileName) throw new Error(`Feishu account ${connection.displayName} is missing its lark-cli profile. Rebind Feishu.`);
...
function resolveFeishuIsoTimeWindow(args: Record<string, unknown>, options: { defaultToday?: boolean } = {}) {
  const now = new Date();
...
function toIsoTime(value: unknown, fallbackMs: number) {
  if (typeof value === 'number' && Number.isFinite(value)) {
  ...
  if (typeof value === 'string' && value.trim()) {
    ...
    if (/^\d+$/.test(trimmed)) {
      const number = Number(trimmed);
      if (Number.isFinite(number)) return new Date(number > 10_000_000_000 ? number : number * 1000).toISOString();
    }
    const parsed = Date.parse(trimmed);
...
function optionalCliArg(flag: string, value: string) {
