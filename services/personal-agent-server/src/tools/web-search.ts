...
export function createWebSearchDynamictool() {
  return {
    ...
    description:
      'Search the public web for current external information. Use this when public web facts, news, industry updates, market information, or web research are needed and no more specific registered channel/tool is better. Returns compact search results with title, URL, and snippet.',
    inputSchema: {
      type: 'object',
      properties: {
...
export function resolveWebSearchProvider(config: ServerConfig): WebSearchProvider | null {
  const configured = config.webSearchProvider;
  if (configured !== 'auto') {
    if (configured === 'serpapi' && !process.env[config.serpApiKeyEnv]?.trim()) return null;
    if (configured === 'serper' && !process.env[config.serperApiKeyEnv]?.trim()) return null;
    if (
    ...
    if (configured === 'bing' && !process.env[config.bingSearchApiKeyEnv]?.trim()) return null;
    return configured;
  }
  if (process.env[config.serpApiKeyEnv]?.trim()) return 'serpapi';
  if (process.env[config.serperApiKeyEnv]?.trim()) return 'serper';
  if (process.env[config.googleCseApiKeyEnv]?.trim() && process.env[config.googleCseIdEnv]?.trim()) {
    return 'google_cse';
  }
  if (process.env[config.bingSearchApiKeyEnv]?.trim()) return 'bing';
  return 'duckduckgo';
}
...
async function runProviderSearch(
  ...
  if (provider === 'serpapi') return serpApiSearch(query, recency, process.env[config.serpApiKeyEnv] || '', config);
  if (provider === 'serper') return serperSearch(query, recency, process.env[config.serperApiKeyEnv] || '', config);
...
async function serpApiSearch(query: string, recency: string | undefined, apiKey: string, config: ServerConfig) {
...
async function serperSearch(query: string, recency: string | undefined, apiKey: string, config: ServerConfig) {
...
async function googleCseSearch(query: string, apiKey: string, cx: string, config: ServerConfig) {
