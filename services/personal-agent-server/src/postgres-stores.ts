...
import {
  type EnqueueMemoryReviewJobInput,
  type MemoryReviewJob,
  type MemoryReviewJobStore,
  type MemoryReviewBillingStatus,
  type MemoryReviewJobStatus,
} from './memory-review-queue.js';
import type { UserProfileEntry, UserProfileSnapshot, UserProfileStore } from './profile-store.js';
...
export async function getPostgresPool(config: ServerConfig): Promise<PgPool> {
  ...
  try {
    ...
    throw new Error(
      `STORAGE_BACKEND=postgres requires the "pg" package. Run npm install in services/personal-agent-server. Original error: ${
  ...
  private async saveProfileEntry(userId: string, content: string, threadId: string | undefined, reason: string, confidence: number) {
    ...
    if (existing.rows[0]) {
      const updated = await pool.query(
        ...
        [String(existing.rows[0].id), threadId || null]
      );
...
export class PostgresMemoryReviewJobStore implements MemoryReviewJobStore {
  ...
  async fail(jobId: string, error: unknown, output?: { stdout?: string; stderr?: string }) {
    ...
    const result = await pool.query(
      ...
      [jobId, message, output?.stdout ? truncate(output.stdout, 8000) : null, output?.stderr ? truncate(output.stderr, 8000) : null]
    );
    return result.rows[0] ? rowToMemoryReviewJob(result.rows[0]) : null;
  }
  ...
  async completeBilling(
    ...
    const result = await pool.query(
      ...
      [jobId, output.status, Math.max(0, Math.round(output.billedCredits))],
    );
    return result.rows[0] ? rowToMemoryReviewJob(result.rows[0]) : null;
  }
  ...
  async listRecent(limit = 50) {
    ...
    const pool = await getPostgresPool(this.config);
    const result = await pool.query(
      'select * from agent_memory_review_jobs order by created_at desc limit $1',
      [Math.max(1, Math.min(200, Math.round(limit)))]
    );
    return result.rows.map(rowToMemoryReviewJob);
  }

  private ensureBillingSchema() {
    if (!this.schemaReady) {
...
function rowToProfileEntry(row: Record<string, unknown>): UserProfileEntry {
  return {
    id: String(row.id || ''),
...
function rowToMemoryReviewJob(row: Record<string, unknown>): MemoryReviewJob {
  return {
    id: String(row.id || ''),
    status: String(row.status || 'queued') as MemoryReviewJobStatus,
    ...
    usage: isRecord(row.usage) ? row.usage as unknown as AgentRunUsage : undefined,
    billingStatus: String(row.billing_status || 'skipped') as MemoryReviewBillingStatus,
...
function dateishToIso(value: unknown) {
