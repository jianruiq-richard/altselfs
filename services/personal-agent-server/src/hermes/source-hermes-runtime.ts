...
function buildCodexDeveloperInstructions(input: { webSearchMode: string; runtimeStateMode: string; message: string }) {
  ...
  if (isCompetitiveIntelligenceMessage(input.message)) {
    instructions.push(
      '',
      'Competitive intelligence task overlay:',
      '- For this turn, operate as Altselfs codex-competitive-intelligence even if the source-runtime path does not expose separate Codex profiles.',
      ...
      '- Use enabled non-local information-source agents and platform/MCP capabilities when available. Semrush is the preferred source for SEO, PPC, keyword, backlink, top-page, and traffic-proxy evidence when enabled.',
