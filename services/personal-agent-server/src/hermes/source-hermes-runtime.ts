...
const ALTSELFS_RUNTIME_CONTEXT_PLUGIN_SOURCE = [
  ...
  '',
  'def _normalize_apiyi_cache(request, api_mode="", provider="", model="", base_url="", **_kwargs):',
  '    hostname = (urlparse(base_url).hostname or "").lower()',
  '    is_apiyi = hostname == "apiyi.com" or hostname.endswith(".apiyi.com")',
  '    if api_mode != "anthropic_messages" or not is_apiyi or "claude" not in model.lower():',
  '        return None',
...
export class HermesSourceRuntime {
  ...
  private spawnHermes(
    ...
    paths: {
      ...
      hermesModelSelection: HermesModelSelection;
      codexModelSelection: CodexModelSelection;
      hermesStableSystemPrompt: string;
      hermesDynamicUserContext: string;
...
export function buildHermesStableSystemPrompt() {
  return [
    ...
    '- HTML deliverables must be a single self-contained `.html` or `.htm` file. Inline CSS, scripts, and data; use data URLs for local media; and do not depend on sibling output files or relative asset paths.',
    '',
    ...
    '',
    'How to delegate:',
    '- Delegate outcomes, not implementation. State the objective, relevant context, constraints, success criteria, target files/entities, and verification expectations. Let Codex choose the tools and execution path.',
    '- For research and reports, first decide the exact evidence needed, analytical method, estimates, conclusions, report structure, and chart requirements yourself. Delegate only bounded evidence collection or exact implementation instructions; do not ask Codex to determine the report substance.',
    '- Do not invent or preselect URLs, citations, image assets, API results, datasets, commands, code snippets, or ranking formulas for Codex unless they came from the user, an uploaded/workspace artifact, or a verified previous tool result.',
    '- For public research or real-world assets, ask Codex to discover and verify sources itself. For images, require verifiable source context such as source page, title, metadata, license, or clear semantic evidence; do not rely on guessed image IDs, guessed CDN URLs, or pixel/color heuristics alone.',
    '- If the user explicitly requests a specific method, pass that method as a constraint. Otherwise avoid prescribing low-level steps such as exact Python code, shell commands, regexes, API call order, or scoring formulas.',
...
export function buildHermesDynamicUserContext(input: {
...
export function emitRuntimeTimingFromStderrLine(
  line: string,
  emitTiming: (type: string, payload: Record<string, unknown>) => void
) {
  for (const timingPrefix of STDERR_TIMING_PREFIXES) {
    ...
    if (
      passthroughType === 'codex.agent_message.delta' ||
