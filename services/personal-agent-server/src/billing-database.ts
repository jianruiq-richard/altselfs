...
export function getBillingPool(config: ServerConfig) {
...
export function getRequiredBillingPool(config: ServerConfig) {
  const pool = getBillingPool(config);
  if (!pool) throw new BillingUnavailableError('BILLING_DATABASE_URL is not configured.');
  return pool;
}

export async function runSerializableBillingTransaction<T>(
  config: ServerConfig,
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt += 1) {
    ...
    try {
      ...
      const code = error && typeof error === 'object' && 'code' in error
        ...
        : '';
      if (!['40001', '40P01', '23505'].includes(code) || attempt === maxAttempts) throw error;
