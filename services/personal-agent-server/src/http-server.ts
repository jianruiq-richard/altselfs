  ...
  return http.createServer(async (req, res) => {
    try {
      ...
      if (req.method === 'POST' && url.pathname === '/internal/artifacts/upload-policy') {
        ...
        if (files.length === 0) return json(res, 400, { error: 'files are required' });
        const maxFiles = readPositiveIntegerEnv('ARTIFACT_OBJECT_STORAGE_MAX_FILES', 6);
        if (files.length > maxFiles) return json(res, 400, { error: `You can upload up to ${maxFiles} files.` });
      ...
      if (req.method === 'POST' && url.pathname === '/internal/artifacts/complete-upload') {
        ...
        if (artifactIds.length === 0) return json(res, 400, { error: 'artifactIds are required' });
        ...
        for (const artifactId of artifactIds) {
          if (!foundIds.has(artifactId)) continue;
      ...
      if (req.method === 'POST' && url.pathname === '/internal/artifacts/download-url') {
        ...
        if (!objectKey) return json(res, 404, { error: 'Artifact does not have an OSS object key' });
        return json(res, 200, {
          ok: true,
      ...
      if (req.method === 'GET' && url.pathname === '/v1/threads/status') {
        if (!config) return json(res, 500, { error: 'config missing' });
        if (!isAgentServiceAuthorized(req, config)) return json(res, 403, { error: 'Forbidden' });
      ...
      if (req.method === 'POST' && url.pathname === '/v1/turns/start') {
        ...
        try {
          ...
          if (error instanceof CreditAdmissionError || error instanceof BillingUnavailableError) {
            return json(res, error.httpStatus, admissionErrorPayload(
              error,
              persisted?.runId || metadataStringValue(turnRequest.metadata, 'runId'),
      ...
      if (req.method === 'POST' && url.pathname === '/v1/turns/start-async') {
        ...
        if (!isAgentServiceAuthorized(req, config)) return json(res, 403, { error: 'Forbidden' });
        const turnRequest = parseTurnStartRequest(body);
        let prepared: Awaited<ReturnType<typeof authorizeAndPersistTurn>>;
        try {
          ...
          if (error instanceof CreditAdmissionError || error instanceof BillingUnavailableError) {
            return json(res, error.httpStatus, admissionErrorPayload(
              error,
              metadataStringValue(turnRequest.metadata, 'runId'),
              turnRequest.threadId,
            ));
          }
          throw error;
        ...
        if (prepared.existing) {
          if (isRejectedTurnStatus(persisted.status)) {
            return json(
              ...
              existingTurnPayload(persisted, turnRequest.threadId),
            );
          }
          return json(res, 202, {
            runId: persisted.runId,
...
async function authorizeAndPersistTurn(
  config: ServerConfig,
  request: TurnStartRequest,
  status: 'QUEUED' | 'RUNNING',
): Promise<{
  persisted: PersistedAgentTurnInput;
  authorization: CreditAuthorization | null;
  existing: boolean;
}> {
...
function rejectionRunStatus(error: CreditAdmissionError | BillingUnavailableError) {
  if (error.code === 'INSUFFICIENT_CREDITS' || error.code === 'SUBSCRIPTION_INACTIVE') {
    return 'REJECTED_CREDITS' as const;
  }
  if (error.code === 'THREAD_BUSY') return 'REJECTED_THREAD_BUSY' as const;
  if (error.code === 'CONCURRENT_TASK_LIMIT') return 'REJECTED_CONCURRENCY' as const;
  return 'REJECTED_BILLING' as const;
}

function isRejectedTurnStatus(status: string | undefined) {
  return Boolean(status?.startsWith('REJECTED_'));
}

function rejectionHttpStatus(code: unknown) {
  if (code === 'INSUFFICIENT_CREDITS' || code === 'SUBSCRIPTION_INACTIVE') return 402;
  if (code === 'THREAD_BUSY') return 409;
  if (code === 'CONCURRENT_TASK_LIMIT') return 429;
  return 503;
}

function existingTurnPayload(persisted: PersistedAgentTurnInput, threadId?: string) {
  return {
    runId: persisted.runId,
...
function admissionErrorPayload(
  error: CreditAdmissionError | BillingUnavailableError,
  runId?: string,
  threadId?: string,
) {
  return {
    ...
    try {
      ...
      write({
        ...
        result: {
          ...
          ...(error instanceof CreditAdmissionError || error instanceof BillingUnavailableError
            ? admissionErrorPayload(
                error,
                persisted?.runId || metadataStringValue(request.metadata, 'runId'),
                request.threadId,
              )
            : {
...
function isAgentServiceAuthorized(req: http.IncomingMessage, config: ServerConfig) {
  if (process.env.OPS_AGENT_TOKEN?.trim()) return isOpsAuthorized(req);
  return config.env !== 'production' && isLoopbackRequest(req);
}

function readRequiredBodyString(body: Record<string, unknown>, key: string) {
...
function normalizeDirectUploadFiles(value: unknown): DirectUploadFile[] {
  ...
  return value
    .map((item) => {
      ...
      if (!name || sizeBytes <= 0) return null;
      return { name, mimeType, sizeBytes };
    })
    .filter((item): item is DirectUploadFile => Boolean(item));
}
...
function publicArtifactRecord(artifact: AgentContextArtifactRecord) {
  const objectKey = artifactMetadataString(artifact.metadata, 'ossObjectKey') || artifactMetadataString(artifact.metadata, 'objectKey');
...
function artifactMetadataString(metadata: Record<string, unknown>, key: string) {
  const value = metadata[key];
  return typeof value === 'string' && value.trim() ? value.trim() : '';
}
  ...
  return {
    ...
    featurePackages: connection.provider === 'feishu'
      ? (Object.prototype.hasOwnProperty.call(connection.metadata || {}, 'feature_packages')
          ...
          : normalizeFeishuCliFeaturePackages(
              connection.metadata?.feature_packages,
...
function findUserResourceByDiskSegment(
  ...
  for (const [key, value] of resources.entries()) {
    if (value.userId && sanitizePathSegment(value.userId) === segment) return { key, value };
  }
  return null;
}
