...
export async function claimNextQueuedAgentTurn(
  ...
  input: {
    workerId: string;
    limits: AgentTurnQueueLimits;
    timeoutMs: number;
  }
): Promise<QueuedAgentTurn | null> {
  const pool = await getRequiredContextPool(config);
  const result = await pool.query(
    [
      'with candidate as (',
...
export async function expireStaleAgentTurns(
  config: ServerConfig,
  input: { staleHeartbeatMs: number }
): Promise<number> {
  const pool = await getRequiredContextPool(config);
  const result = await pool.query(
    [
...
export async function markQueuedAgentTurnWaiting(
  config: ServerConfig,
  input: { runId: string; reason: string; delayMs: number }
) {
  const pool = await getRequiredContextPool(config);
  await pool.query(
    [
    ...
    [input.runId, input.reason, Math.max(1_000, input.delayMs)]
  );
}
...
export async function getAgentThreadRuntimeStatus(
  config: ServerConfig,
  input: { threadId: string; investorId?: string; userId?: string; runId?: string; recentEventLimit?: number }
  ...
  if (requestedRunId && !runs.some((run) => run.id === requestedRunId)) {
    const values: unknown[] = [input.threadId, requestedRunId];
    ...
    if (requestedRunResult.rows[0]) runs = [requestedRunResult.rows[0], ...runs];
  }
...
export async function getAgentContextOpsSingleUserUsage(
  config: ServerConfig,
  investorId: string
): Promise<AgentContextOpsUserUsage | null> {
  if (!config.contextDatabaseUrl || !investorId.trim()) return null;
  const pool = await getRequiredContextPool(config);
  const result = await pool.query(`
    with usage_rows as (
      select
...
function rowToQueuedAgentTurn(row: Record<string, unknown>): QueuedAgentTurn {
  ...
  if (!request) throw new Error(`Queued agent run ${runId || '(unknown)'} is missing an executable request`);
...
function requestForExecutionStorage(request: TurnStartRequest) {
  return {
    userId: request.userId,
...
function storedTurnRequest(value: unknown): TurnStartRequest | null {
...
function buildArtifactContextMessage(input: { artifacts: Array<Record<string, unknown>> }) {
