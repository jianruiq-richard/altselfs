...
export async function authorizeAgentRun(
  config: ServerConfig,
  input: {
    investorId: string;
    threadId: string;
  ...
  return runSerializableBillingTransaction(config, async (client) => {
    ...
    if (activeReservations.rows.length >= plan.concurrentTasks) {
      throw new CreditAdmissionError(
        ...
        `${plan.name} allows ${plan.concurrentTasks} active task${plan.concurrentTasks === 1 ? '' : 's'}.`,
        {
    ...
    if (config.creditsEnforcementMode === 'enforce' && availableCredits < holdCredits) {
      throw new CreditAdmissionError(
        ...
        `${holdCredits} available credits are required to authorize this task.`,
        {
    ...
    if (reservedCredits > 0) {
      await client.query(
        ...
        [refreshedAccount.id, nextReservedCredits],
      );
    }
...
export async function getBillingCapacity(config: ServerConfig, investorId: string) {
  return runSerializableBillingTransaction(config, async (client) => {
    const now = new Date();
...
async function ensureFreeSubscription(client: PoolClient, investorId: string) {
  await client.query(
    ...
    [id('credit_subscription'), 'FREE', 'ACTIVE', PLAN_LIMITS.FREE.monthlyCredits, investorId],
  );
}
...
async function releaseExpiredReservations(client: PoolClient, account: AccountRow, now: Date) {
  const expired = await client.query(
    ...
    [account.id, 'ACTIVE', now],
  );
  if (expired.rows.length === 0) return account;

  const releasedCredits = expired.rows.reduce(
    (sum, row) => sum + Math.max(0, numberValue(row.reservedCredits)),
    0,
  );
  const nextReservedCredits = Math.max(0, account.reservedCredits - releasedCredits);
  await client.query(
    'update credit_accounts set "reservedCredits" = $2, "updatedAt" = now() where id = $1',
    [account.id, nextReservedCredits],
  );
  await client.query(
    'update credit_reservations set status = $2, "settledAt" = $3, "updatedAt" = now() where id = any($1::text[])',
    [expired.rows.map((row) => String(row.id)), 'EXPIRED', now],
  );
...
async function authorizationFromExisting(
  config: ServerConfig,
  ...
  const investorResult = await client.query(
    'select "investorId" from credit_reservations where "runId" = $1',
    [runId],
  );
  const investorId = String(investorResult.rows[0]?.investorId || '');
  const activeCountResult = await client.query(
    'select count(*)::int as count from credit_reservations where "investorId" = $1 and status = $2 and "expiresAt" > $3',
    [investorId, 'ACTIVE', now],
  );
