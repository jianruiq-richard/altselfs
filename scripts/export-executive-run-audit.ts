...
async function main() {
  ...
  const markdown = [
    ...
    '',
    'This export includes saved raw prompts and raw completions when they were captured, but it may not cover every planner, summary, structuring, or reply step.',
    '',
    'Inspect the raw JSON for the complete persisted payload, planner trace, subagent outputs, tool calls, generated document, assistant reply, source metadata, and profile data.',
    '',
    '## User Request',
    ...
    '',
    [
      '- If subagent results or tool outputs are missing, compare `Subagent Results` with `WeChat tool Calls`.',
      '- If the final briefing is thin, inspect the captured raw prompt and raw output for `generate_briefing_summary` and `structure_briefing_json`.',
      '- If source selection looks wrong, review the planner decision and selected source list.',
      '- If the model failed, check the prompt, model, raw output, duration, and error fields.',
    ].join('\n'),
    '',
    '## Markdown Compact Preview',
    '',
    'Compact preview of the generated document. Use the raw JSON for full fidelity.',
